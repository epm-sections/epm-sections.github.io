...
Given a random variable _Y_ ~ _f_~_Y_~(_y_), we seek the distribution of _U_ := g(_Y_) for some real-valued function _g_.
:::
...
-   So, to identify the distribution of _U_, we can either find: its CDF ([**CDF Method**]{.alert}), PDF ([**Method of Transformations**]{.alert}), or MGF ([**MGF Method**]{.alert})

::: {.fragment}
::: {.callout-tip}
## **Example 1: Guiding Example**

Let _Y_ ~ Exp(β) for some β > 0, and define _U_ := _c_ _Y_ for some _c_ > 0.
:::
:::
...
-   Often a three-step procedure:
    1)   Write $F_U(u) := \Prob(U \leq u) = \Prob(g(Y) \leq u)$
    2)   Manipulate the event $\{g(Y) \leq u\}$ to be of the form $\{Y \in B_u\}$ for some set _B_~_u_~
    3)   Use the PDF of _Y_ (which is known!) to evaluate $\Prob(B_u)$, thereby finding an expression for the CDF _F_~_U_~(_u_) of _U_.
...
::: {.nonincremental}
1)    **Step 1:** $F_U(u) := \Prob(U \leq u) = \Prob( \ \ \ \ \ \ \ \ \ \ \leq u)$ <br /><br />

2)    **Step 2:** $\{ \ \ \ \ \ \ \ \ \ \ \leq u \} =$ <br /><br />
...
2)    The underlying transforamtion is [**strictly monotonic**]{.alert} (can anyone tell me why?)
:::
:::
...
::: {.nonincremental}
-   **Transformation:** $g(y) =$  <br /><br />

-   **Inverse Transformation:** $g^{-1}(u) =$ <br /><br />

-   **Derivative of Inverse:** $\left| \frac{\mathrm{d}}{\mathrm{d} u} g^{-1}(u) \right| =$ <br /><br />
...
::: {.nonincremental}
-   By a previously-derived formula, $M_U(t) = M_Y(      )$ <br /><br />

-   The MGF of _Y_, which we can find \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ , is given by <br /><br /> <br /><br />
 
-   Therefore, the MGF of _U_ is given by <br /><br />

-   We recognize the MGF of _U_ as that of the \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ distribution, allowing us to conclude that _U_ ~ 
:::
...
$$ \overline{Y}_n := \frac{1}{n} \sum_{i=1}^{n} Y_i; \qquad S_n^2 := \frac{1}{n - 1} \sum_{i=1}^{n} (Y_i - \overline{Y}_n)^2 $$
...
::: {.nonincremental}
1)    $\overline{Y}_n \sim \mathcal{N}\left( \mu, \ \frac{\sigma^2}{n} \right)$
...
## {{< fa vial >}} Sampling
### _t_- and _F_-distributions

::: {.fragment}
::: {.callout-note}
## **Definition:** _t_-Distribution

If $Z \sim \mathcal{N}(0, 1)$ and $U \sim \chi^2_{\nu}$ with $Z \perp U$, then
$$ T := \frac{Z}{\sqrt{U / \nu}} \sim t_{\nu} $$
:::
:::
...
::: {.fragment}
::: {.callout-note}
## **Definition:** _F_-Distribution

If $U_1 \sim \chi^2_{\nu_1}$ and $U_2 \sim \chi^2_{\nu_2}$ with $U_1 \perp U_2$, then
$$ F := \frac{U_1 / \nu_1}{U_2 / \nu_2} \sim F_{\nu_1, \nu_2} $$
:::
:::
...
Let $Y_1, \cdots, Y_n$ denote an i.i.d. sample from a distribution with mean $\mu$ and finite variance $\sigma^2 < \infty$. Then
$$ \frac{\sqrt{n}(\overline{Y}_n - \mu)}{\sigma} \distto \mathcal{N}(0, 1) $$
which we can sometimes write as, for sufficiently large _n_,
$$ \overline{Y}_n \stackrel{\cdot}{\sim} \mathcal{N}\left( \mu , \ \frac{\sigma^2}{n} \right) \sim \mathcal{N}\left( \E[Y_i], \ \frac{\Var(Y_i)}{n} \right) $$
:::

-   The symbol $\stackrel{\cdot}{\sim}$ can be read as "approximately distributed as"

-   [https://epm027.shinyapps.io/CLT_Gamma/](https://epm027.shinyapps.io/CLT_Gamma/)
...
Let $X_n \sim \mathrm{Bin}(n, p)$. Then
$$ \frac{X_n - np}{\sqrt{np(1 - p)}} \distto \mathcal{N}\left( 0, 1 \right) $$
More specifically, provided that $n > p \cdot (\max\{p, 1 - p\} / \min\{p, 1 - p\})$ then
$$ X_n \stackrel{\cdot}{\sim} \mathcal{N}\left( np, \ np(1 - p) \right) $$
:::
...
# Estimation {background-color="black" background-image="https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExM3lhdGY0NXhiYnlzZ2V3Y3g0bGMxdnRrYWJudWZ1NWRxcGt0YjB4biZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/WRQBXSCnEFJIuxktnw/giphy.gif" background-size="100rem"}
