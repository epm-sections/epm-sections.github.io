...
[$$ \beta_2(\mu_A) > \beta_1(\mu_A) \qquad \forall \mu_A \in \Theta_A $$]{.fragment}
...
-   Suppose in a test of $H_0: \theta \in \Theta_0$ vs $H_A: \theta \in \Theta_A$, there exists a test with power function $\beta(\cdot)$ such that
$$ \beta(\theta_A) > \beta^{\ast}(\theta_A) \qquad \forall \beta^{\ast}, \ \forall \theta_A \in \Theta_A $$

-   That is, among _all_ other tests (with corresponding power functions $\beta^{\ast}$), our original test has higher power _for any value in the alternative space_.
...
$$ \left\{ \frac{\max\limits_{\theta \in \Theta_0}\Lik(\theta; X_1, \cdots, X_n)}{\max\limits_{\theta \in \Theta_A}\Lik(\theta_A; X_1, \cdots, X_n)} < k  \right\} $$
