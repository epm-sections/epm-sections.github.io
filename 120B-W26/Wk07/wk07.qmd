...
-   If $\theta = 0.4$, how likely were we to have observed what we saw?
    -   If $\theta = 0.4$, the probability of observing three heads is $\Prob(X = 3) = \binom{5}{3}(0.4)^3(0.6)^2 \approx 0.2304$
...
\begin{align*}
  \class{fragment}{{} \mathcal{L}(\theta; x_1, x_2, \cdots, x_n)}
    ...
    &\class{fragment}{{} = \prod_{i=1}^{n} \left[ \frac{1}{\theta} e^{-x_i/\theta} \cdot 1 \! \! 1_{\{x_i \geq 0\}} \right] }    \\[3px]
...
-   Procedurally, we often work with the [**log-likelihood**]{.alert} instead. That is:
   ...
   3)   Find the maximizing value of the log-likelihood (including checking the second derivative)
